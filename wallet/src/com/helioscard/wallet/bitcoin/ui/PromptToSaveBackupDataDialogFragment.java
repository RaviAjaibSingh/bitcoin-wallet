...
public class PromptToSaveBackupDataDialogFragment extends DialogFragment {
    ...
    private Wallet _walletFromFile;
    
    private PromptToSaveBackupDataDialogFragment(String sourceCardIdentifier, List<ECKeyEntry> listOfKeys, String password, Wallet walletFromFile) {
    	_sourceCardIdentifier = sourceCardIdentifier;
    	_listOfKeys = listOfKeys;
    	_password = password;
    	_walletFromFile = walletFromFile;
    }

	public static void prompt(FragmentManager fragmentManager, String sourceCardIdentifier, List<ECKeyEntry> listOfKeys, String password, Wallet walletFromFile) {
		PromptToSaveBackupDataDialogFragment frag = new PromptToSaveBackupDataDialogFragment(sourceCardIdentifier, listOfKeys, password, walletFromFile);
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        alertDialogBuilder.setMessage(getResources().getString(_sourceCardIdentifier != null ? R.string.nfc_aware_activity_prompt_to_save_backup_data_dialog_message : R.string.nfc_aware_activity_prompt_to_save_backup_data_dialog_message_only_card));

        if (_sourceCardIdentifier != null) {
    ...
    public Wallet getWalletFromFile() {
    	return _walletFromFile;
    }
    ...
    private void handleSaveToFile(String password) {
		...
		for (int i = 0; i < _listOfKeys.size(); i++) {
			...
			ecKey.setCreationTimeSeconds(ecKeyEntry.getTimeOfKeyCreationSecondsSinceEpoch());
			wallet.addKey(ecKey);
		}
